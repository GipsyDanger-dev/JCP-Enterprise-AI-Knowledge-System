...
export function RequiredRead({ title, category, status, progress, onClick }: { title: string; category: string; status: string; progress: number; onClick?: () => void }) {
  return (
    <div className="required-item" onClick={onClick} role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined}>
      ...
      <div className="reading-progress"><div><i style={{ width: `${progress}%` }} /></div><span>{status}</span></div>
