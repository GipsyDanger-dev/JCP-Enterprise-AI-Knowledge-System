...
export function UploadModal({ open, onClose, onUploaded }: UploadModalProps) {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!extension || !allowed.includes(extension)) {
      setFile(null)
      setError(isId ? 'Hanya file PDF, DOCX, dan TXT yang didukung.' : 'Only PDF, DOCX, and TXT files are supported.')
      return
    }
    ...
    <div className="modal-overlay" onClick={handleClose}>
      <div className="modal-card upload-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="upload-file-area" onClick={() => !uploading && fileRef.current?.click()}>              <input ref={fileRef} type="file" accept=".pdf,.docx,.txt" onChange={handleFileChange} style={{ display: 'none' }} />
          {file ? (
            ...
            <div className="upload-file-empty">
              ...
              <small>{isId ? 'PDF, DOCX, atau TXT, maks 10MB' : 'PDF, DOCX, or TXT, max 10 MB'}</small>
            </div>
          )}
        </div>
