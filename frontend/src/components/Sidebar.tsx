  ...
  return (
    <aside className={[menuOpen ? 'sidebar open' : 'sidebar', collapsed ? 'collapsed' : ''].filter(Boolean).join(' ')}>
      ...
      <nav aria-label="Primary navigation">
        ...
        {user?.role === 'ADMIN' && <NavLink to="/activity" title={collapsed ? (isId ? 'Log aktivitas' : 'Activity log') : undefined} className={({ isActive }) => (isActive ? 'nav-item active' : 'nav-item')} onClick={onClose}><Activity size={18} />{!collapsed && <span>{isId ? 'Log aktivitas' : 'Activity log'}</span>}</NavLink>}
