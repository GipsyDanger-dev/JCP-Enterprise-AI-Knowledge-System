...
export class RequiredReadingsService {
  ...
  async assign(documentId: string, userIds: string[], dueAtInput?: string) {
    ...
    const users = await this.prisma.user.findMany({ where: { id: { in: uniqueUserIds }, accountType: AccountType.COMPANY, isAdmin: false, isActive: true }, select: { id: true, displayName: true, employeeNumber: true } });
  ...
  async complete(id: string, userId: string) {
    ...
    const admins = await this.prisma.user.findMany({ where: { accountType: AccountType.COMPANY, isAdmin: true, isActive: true }, select: { id: true } });
